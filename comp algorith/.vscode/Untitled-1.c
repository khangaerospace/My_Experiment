...
int find_height(bag_t *bag){
    if(bag == NULL){
        return 0;
    }
    return 1 + max(height(bag->root->left),height(bag->root->right));
}
bool is_avl_tree(bag_t *bag){
    if (bag == NULL){
        return true;
    }
    int lh = height(bag->root->left);
    int rh =height(bag->root->right);
    if(abs(lh-rh)<= 1 && is_avl_tree(bag->root->left) && is_avl_tree(bag->root->right)){
        return true;
    }

...
void brighter(struct rgb_img *im, double level){
    for(int y; y < im->height; y++){
        for(int x; x< im->width; x++ ){
            int r = get_pixel(im,y,x, 0)*level;
            int g = get_pixel(im, y,x,1)*level;
            int b = get_pixel(im, y,x,2)*level;
...
int main(void)
{
    ...
    read_in_img(&im, "new.bin");
    brighter(im, 0.001);
    //set_pixel(&im,3,3,3,3,3);
    //printf("Height: %d, Width: %d\n", im->height, im->width);
    write_img(im,"newnew.bin");
    destroy_image(im);
}
